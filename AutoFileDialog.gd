...
func showDialog(fileMode: FileDialog.FileMode, ok: Callable, filter: Array[String] = ["*.* ; All Files"], file := "file" + str(Random.next(1000000)), path := OS.get_system_dir(OS.SYSTEM_DIR_DESKTOP)) -> void:
